...
import bitcoinscript.codigos.OpcodesControl; 
...
public class InterpretadorScript {
    ...
    public boolean validar(String scriptSig, String scriptPubKey, boolean traza) {
        ...
        try {
            ...
            if (traza) System.out.println("  ✗ ERROR DE EJECUCIÓN: " + e.getMessage());
            return false; 
        }

        PilaScript pila = ctx.getPilaP();
        if (pila.estaVacia() || !ctx.debeEjecutarInstruccionNormal()) {
            if (traza) System.out.println("  ✗ Pila vacía o bloque OP_IF sin cerrar");
            return false;
        }
        ...
        for (TokenScript token : tokens) {
            if (token.esDato()) {
                if (ctx.debeEjecutarInstruccionNormal()) {
                    ...
                    ctx.trazar("PUSH (Omitido)");
                }
                continue;
            }

            CodigoOperacion op = registro.obtener(token.getCodigoOp());
            if (op instanceof OpcodesControl.OpIf || op instanceof OpcodesControl.OpNotIf || 
                op instanceof OpcodesControl.OpElse || op instanceof OpcodesControl.OpEndIf) {
                op.ejecutar(ctx);
            } else {
                if (ctx.debeEjecutarInstruccionNormal()) op.ejecutar(ctx);
                else ctx.trazar(op.getNombre() + " (Omitido)");
            }
        }
    }
}
